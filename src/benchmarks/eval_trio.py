...
def load_callset(path):
    ...
    for line in file:
        ...
        if info["SVTYPE"] in ["DEL", "INS", "DUP", "INV"]:
            ...
            callset[info["SVTYPE"]].append(
                [chr, pos, info["END"], info["SVLEN"], phase_GT(seq[9]), 0]
            )
        ...
        if info["SVTYPE"] == "BND":
            ...
            callset[info["SVTYPE"]].append(
                [chr, pos, info["CHR2"], info["END"], form, phase_GT(seq[9]), 0]
            )
            ...
            for i in call_B[svtype]:
                ...
                for j in call_A[svtype]:
                    ...
                    if svtype == "INS":
                        if (
                            abs(i[1] - j[1]) <= offect
                            and float(min(i[3], j[3]) / max(i[3], j[3])) >= bias
                        ):
                            i[-1] = 1
                    elif svtype == "BND":
...
def main_ctrl(args):
    ...
    for i in svtype:
        ...
        logging.info(
            "%s-%s: %d\t%d\t%.2f."
            % ("F1", i, record, true_record, 100 * float(true_record / record))
        )
        record, true_record = statistics_true_possitive(call_father, i, ["hom"])
        logging.info(
            "%s-%s: %d\t%d\t%.2f."
            % ("MP", i, record, true_record, 100 * float(true_record / record))
        )
        record, true_record = statistics_true_possitive(call_mother, i, ["hom"])
        logging.info(
            "%s-%s: %d\t%d\t%.2f."
            % ("FP", i, record, true_record, 100 * float(true_record / record))
        )
