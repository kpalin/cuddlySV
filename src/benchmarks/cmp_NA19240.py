...
def parseArgs(argv):
    ...
    parser.add_argument(
        "-o",
        "--offect",
        help="Offect of breakpoint distance.[%(default)s]",
        default=1000,
        type=int,
    )
        ...
        if info["SVLEN"] >= 50 and info["SVLEN"] <= 100000:
            if ALT == "INV":
                ...
                if len(last_inv):
                    ...
                    cuteSV_call[last_inv[0][0]][last_inv[0][1]].append(
                        [last_inv[0][2], last_inv[0][3], last_inv[0][4], 0]
                    )
                    last_inv = list()
    file.close()
    return cuteSV_call
...
def load_sniffles(sniffles_path):
    ...
    for line in file:
        ...
        if info["SVLEN"] >= 50 and info["SVLEN"] <= 100000:
            if info["SVTYPE"] == "INV":
                last_inv.append(
                    [info["SVTYPE"], chr, pos, info["SVLEN"], info["END"], info["RE"]]
                )
            else:
                sniffles_call[info["SVTYPE"]][chr].append(
                    [pos, info["SVLEN"], info["END"], 0]
                )
                if len(last_inv):
                    last_inv = sorted(last_inv, key=lambda x: -x[3])
                    sniffles_call[last_inv[0][0]][last_inv[0][1]].append(
                        [last_inv[0][2], last_inv[0][3], last_inv[0][4], 0]
                    )
                    last_inv = list()
...
def load_pbsv(pbsv_path):
    ...
    for line in file:
        ...
        if info["SVTYPE"] == "INV":
            pbsv_call[info["SVTYPE"]][chr].append(
                [pos, info["END"] - pos + 1, info["END"], 0]
            )
        else:
            if info["SVLEN"] >= 50 and info["SVLEN"] <= 100000:
                pbsv_call[info["SVTYPE"]][chr].append(
...
def cmp_callsets(base, call, flag, Bias, Offect):
                ...
                if chr not in call[svtype]:
                    ...
                    for i in base[svtype][chr]:
                        for j in call[svtype][chr]:
                            if (
                                i[0] - Offect <= j[0] <= i[2] + Offect
                                or i[0] - Offect <= j[2] <= i[2] + Offect
                                or j[0] - Offect <= i[0] <= j[2] + Offect
                            ):
    ...
    logging.info(
        "F-measure: %.2f"
        % (200.0 * tp_base * tp_call / (total_base * tp_call + tp_base * total_call))
    )
